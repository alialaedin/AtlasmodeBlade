...
<div class="page-header">
	<x-breadcrumb :items="[
		['title' => 'لیست دسته بندی محصولات', 'route_link' => 'admin.categories.index'], 
			...
			<div class="row">
					...
					<div class="col-12 col-lg-4 col-md-6">
						<div class="form-group">
							...
							<input type="text" id="title" class="form-control" name="title" value="{{ old('title', $category->title) }}" required autofocus />
							<span class="text-muted-dark mt-2 mr-1 font-weight-bold fs-11">عنوان را حتما به <span class="text-danger">فارسی</span> وارد کنید!</span>
						</div>
					</div>

					<div class="col-12 col-lg-4 col-md-6">
						<div class="form-group">
							<label for="en_title" class="control-label"> عنوان: <span class="text-danger">&starf;</span></label>
							<input type="text" id="en_title" class="form-control" name="en_title" value="{{ old('en_title', $category->en_title) }}" />
							<span class="text-muted-dark mt-2 mr-1 font-weight-bold fs-11">عنوان را حتما به <span class="text-danger">انگیلیسی</span> وارد کنید!</span>
						</div>
					</div>
						...
						<div class="form-group">
							...
							<select class="form-control" name="parent_id" id="parent_id">
								...
								@foreach ($categories ?? [] as $category1)
									<option value="{{ $category1->id }}" {{ old('parent_id', $category->parent_id) || request('parent_id') == $category1->id ? 'selected' : '' }}>{{ $category1->title }}</option>
								@endforeach
						</select>
						</div>
					</div>
