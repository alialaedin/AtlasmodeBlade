<header class="header px-4 px-md-8 px-3xl-0 pb-1 bg-gray-100 position-relative">
  <div class="header-top container-2xl d-flex justify-content-between align-items-sm-center pb-2 pt-3 pt-lg-5">
		...
		<div class="d-flex align-items-center gap-1 mt-lg-0">
			...
			<!-- User -->
			@if ($user === null)
				<a href="{{ route('customer.my-account') }}" class="d-none d-lg-block">
			...
			<!-- Shipping Cart -->
			<a href="{{ route('customer.carts.index') }}" class="shopping-cart position-relative">
				...
				<span class="position-absolute carts-count-span radius-circle text-center text-button-1 bg-primary-500 color-white w-4 h-4"></span>
			</a>
...
@push('scripts')
	<script>
		$(document).ready(() => {
			...
			const isLogin = @json($user) !== null;
			const cartsCount = isLogin ? @json($cartsCount) : getCartCookie().length;
			$('.carts-count-span').each(function(index, span) {
